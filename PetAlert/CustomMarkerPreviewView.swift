//
//  CustomMarkerPreviewView.swift
//  PetAlert
//
//  Created by Claudia on 30.01.2018.
...
class CustomMarkerPreviewView: UIView {
    ...
    func setData(title: String, img: UIImage, breed: String) {
        lblTitle.text = title
        imgView.image = img
        lblBreed.text = breed
    }
    ...
    func setupViews() {
        ...
        addSubview(lblBreed)
        lblBreed.centerXAnchor.constraint(equalTo: centerXAnchor).isActive=true
        lblBreed.centerYAnchor.constraint(equalTo: imgView.centerYAnchor).isActive=true
        lblBreed.widthAnchor.constraint(equalToConstant: 300).isActive=true
        lblBreed.heightAnchor.constraint(equalToConstant: 40).isActive=true
    }
    ...
    let imgView: UIImageView = {
        let v=UIImageView()
        v.image=#imageLiteral(resourceName: "2_rottweiler")
        v.translatesAutoresizingMaskIntoConstraints=false
